...
import numpy as np
import pandas as pd
from balance_assessment_radar.preprocessing.scripts.archived.pose_vectors import pose_df

class TSVProcessorKnee:
    def __init__(self, input_folder, participant_id, output_folder="simple_csv"):
    ...
    def process_tsv(self, file_path):
        with open(file_path, mode='r', newline='') as tsv_file:
            ...
            if df.shape[0] != 4000 or df.shape[1] != 72:
                ...
                # )
                self.output_folder = "/Users/danielcopeland/Library/CloudStorage/OneDrive-MassachusettsInstituteofTechnology/Yoga_Study_MOCAP_Data/all_MOCAP_data"
                output_file_path = os.path.join(
                    self.output_folder, os.path.splitext(os.path.basename(file_path))[0] + ".csv"
                )
